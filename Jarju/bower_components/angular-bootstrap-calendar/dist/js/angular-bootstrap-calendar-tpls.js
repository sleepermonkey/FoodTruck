...
angular
  ...
  .factory('calendarHelper', ["$q", "$templateRequest", "dateFilter", "moment", "calendarConfig", function($q, $templateRequest, dateFilter, moment, calendarConfig) {
    ...
    function getBuffetBadgeTotal(events) {
        return events.filter(function (event) {
            return event.incrementsBadgeTotal !== false && event.type == 'buffet';
        }).length;
    }

    function getFoodBoxBadgeTotal(events) {
        return events.filter(function (event) {
            return event.incrementsBadgeTotal !== false && event.type == 'foodbox';
        }).length;
    }

    function getCoffeeBreakBadgeTotal(events) {
        return events.filter(function (event) {
            return event.incrementsBadgeTotal !== false && event.type == 'coffeebreak';
        }).length;
    }

    function getOtherBadgeTotal(events) {
        return events.filter(function (event) {
            return event.incrementsBadgeTotal !== false && event.type != 'buffet' && event.type != 'foodbox' && event.type != 'coffeebreak';
        }).length;
    }
    ...
    function getMonthView(events, viewDate, cellModifier) {
      ...
      view.days = view.days.map(function(day) {
        ...
        day.BuffetBadgeTotal = getBuffetBadgeTotal(day.events);
        day.FoodBoxBadgeTotal = getFoodBoxBadgeTotal(day.events);
        day.CoffeeBreakBadgeTotal = getCoffeeBreakBadgeTotal(day.events);
        day.OtherBadgeTotal = getOtherBadgeTotal(day.events);
